...
<style>
    ...
    /* ── Topbar ── */
    .topbar { padding: 24px 32px 16px 32px; display: flex; align-items: flex-end; justify-content: space-between; }
...
<div class="page-wrapper">
    ...
    <div class="topbar">
        <div>
            ...
            <div class="topbar-sub">Resumen general de la sucursal</div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;">
            <span class="topbar-date">{{ now()->isoFormat('dddd D [de] MMMM, YYYY') }}</span>
            {{-- TODO: conectar exportación de reporte --}}
    ...
    {{-- ================================================================
         KPI ROW
         TODO (controlador): pasar array $kpis con las siguientes claves:
           - cartera_activa         (decimal) → suma linea_credito de distribuidoras activas
           - distribuidoras_activas (int)     → count distribuidoras donde estado = 'activa'
           - cartera_morosa         (decimal) → suma total_deuda de cortes vencidos de morosas
           - distribuidoras_morosas (int)     → count distribuidoras donde estado = 'morosa'
           - cortes_pendientes      (int)     → count cortes donde estado = 'pendiente'
           - cortes_vencidos        (int)     → count cortes donde estado = 'vencido'
           - puntos_total           (int)     → sum puntos_saldo de todas las distribuidoras
           - valor_punto            (decimal) → valor de configuraciones donde clave = 'valor_punto'
         ================================================================ --}}
    <div class="kpi-row">
        ...
        <div class="kpi-card blue-left">
            ...
            <div class="kpi-value mono">
                ${{ number_format($kpis['cartera_activa'] ?? 0, 0, '.', ',') }}
            </div>
            <div class="kpi-sub blue">
                {{ $kpis['distribuidoras_activas'] ?? 0 }} distribuidoras activas
            </div>
        </div>
        ...
        <div class="kpi-card red-left">
            ...
            <div class="kpi-value mono">
                ${{ number_format($kpis['cartera_morosa'] ?? 0, 0, '.', ',') }}
            </div>
            <div class="kpi-sub red">
                {{ $kpis['distribuidoras_morosas'] ?? 0 }} distribuidoras en mora
            </div>
        </div>
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value mono">
                {{ number_format($kpis['puntos_total'] ?? 0, 0, '.', ',') }}
            </div>
            <div class="kpi-sub muted">
                = ${{ number_format(($kpis['puntos_total'] ?? 0) * ($kpis['valor_punto'] ?? 2), 0, '.', ',') }} en pts
            </div>
        </div>
    ...
    {{-- ================================================================
         FILA 1 — Comportamiento de pagos + columna lateral
         ================================================================ --}}
    <div class="content-grid">

        {{-- ── Tabla comportamiento ──────────────────────────────────────
             TODO (controlador): pasar colección $distribuidoras.
             Cada modelo necesita los siguientes atributos:
               $d->persona->nombre / apellido
               $d->categoria->nombre / porcentaje_comision
               $d->referencia_actual   → corte pendiente actual (nullable)
               $d->pagos_tiempo        → int (pagos dentro del plazo)
               $d->pagos_total         → int (total de cortes pagados o vencidos)
               $d->pagos_anticipados   → int
               $d->pagos_tardios       → int
               $d->estado              → 'activa' | 'morosa' | 'inactiva'
               $d->id                  → para los links de acción
        --}}
        <div class="main-card">
            <div class="card-header-row">
                <span class="card-header-title">Comportamiento de pagos por distribuidora</span>
                {{-- TODO: ruta gerente.distribuidoras --}}
                <a href="#" class="card-header-action">Ver historial completo →</a>
            </div>
            <table class="pf-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="mono" style="color:#16A34A;">
                            {{ $d->pagos_tiempo }}/{{ $d->pagos_total }}
                        </td>
                        <td class="mono" style="color:#2563EB;">
                            {{ $d->pagos_anticipados }}
                        </td>
                        <td class="mono" style="color:{{ $d->pagos_tardios > 0 ? '#DC2626' : '#94A3B8' }};">
                            {{ $d->pagos_tardios }}
                        </td>
                        <td>
                            @if($d->estado === 'activa')
                            ...
                            @elseif($d->estado === 'morosa')
                                <span class="badge badge-red">Morosa</span>
                            @else
                                <span class="badge badge-gray">Inactiva</span>
                            @endif
                        </td>
                        <td>
                            {{-- TODO: reemplazar # con ruta gerente.distribuidoras.show --}}
                    ...
                    <tr>
                        <td colspan="7" class="empty-state">
                            Los datos aparecerán aquí cuando se conecte la base de datos.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            {{-- ── Morosidad global ────────────────────────────────────────
                 TODO (controlador): pasar array $morosidad con las claves:
                   - distribuidoras (int)
                   - monto          (decimal)
                   - clientes       (int)
                   - porcentaje     (float)
            ...
            <div class="main-card">
                <div class="card-header-row">
                    <span class="card-header-title">Morosidad global</span>
                </div>
                <div class="stat-grid">
                    <div class="stat-cell">
                        <div class="stat-label">Dist. morosas</div>
                        <div class="stat-val red">{{ $morosidad['distribuidoras'] ?? 0 }}</div>
                    </div>
                    <div class="stat-cell">
                        <div class="stat-label">Monto en mora</div>
                        <div class="stat-val red">
                            ${{ number_format($morosidad['monto'] ?? 0, 0, '.', ',') }}
                        </div>
                    </div>
                    <div class="stat-cell">
                        <div class="stat-label">Clientes morosos</div>
                        <div class="stat-val amber">{{ $morosidad['clientes'] ?? 0 }}</div>
                    </div>
                    <div class="stat-cell">
                        <div class="stat-label">% cartera riesgo</div>
                        <div class="stat-val amber">
                            {{ number_format($morosidad['porcentaje'] ?? 0, 1) }}%
                        </div>
                    </div>
                </div>
            </div>
            ...
            {{-- ── Notificaciones ──────────────────────────────────────────
                 TODO (controlador): pasar colección $notificaciones.
                 Cada modelo necesita:
                   $notif->tipo       → 'mora' | 'pago' | 'presolicitud' | 'vencimiento'
                   $notif->mensaje    → string
                   $notif->created_at → Carbon (para diffForHumans)
            --}}
            <div class="main-card">
                <div class="card-header-row">
                    <span class="card-header-title">Notificaciones</span>
                    {{-- TODO: ruta gerente.notificaciones --}}
                    <a href="#" class="card-header-action">Ver todas →</a>
                </div>
                @forelse($notificaciones ?? [] as $notif)
                <div class="notif-item">
                    <div class="notif-dot
                        {{ $notif->tipo === 'mora'         ? 'red'   : '' }}
                        {{ $notif->tipo === 'pago'         ? 'green' : '' }}
                        {{ $notif->tipo === 'presolicitud' ? 'blue'  : '' }}
                        {{ $notif->tipo === 'vencimiento'  ? 'amber' : '' }}">
                    </div>
                    <div>
                ...
                @empty
                <div class="empty-state" style="padding:20px 18px;">
    ...
    {{-- ================================================================
         FILA 2 — Saldo cortes + Puntos + Presolicitudes
         ================================================================ --}}
    <div class="content-grid three-col" style="padding-bottom:32px;">

        {{-- ── Saldo de cortes activos ──────────────────────────────────
             TODO (controlador): pasar colección $saldo_cortes.
             Cada item necesita:
               $sc->nombre      → string (nombre de la distribuidora)
               $sc->total_deuda → decimal
               $sc->estado      → 'activa' | 'morosa'
               $sc->porcentaje  → int 0-100 (relativo al mayor saldo del listado)
        --}}
        <div class="main-card">
            <div class="card-header-row">
                <span class="card-header-title">Saldo de cortes activos</span>
                {{-- TODO: ruta gerente.cortes --}}
                <a href="#" class="card-header-action">Ver todos →</a>
            </div>
            <div style="padding:16px 22px;">
                ...
                <div class="bar-row">
                    ...
                    <div class="bar-track">
                        <div class="bar-fill {{ $sc->estado === 'morosa' ? 'red' : '' }}"
                ...
                @empty
                <p class="empty-state" style="padding:16px 0;">Sin cortes activos.</p>
                @endforelse
            </div>
        </div>

        {{-- ── Saldo de puntos ──────────────────────────────────────────
             TODO (controlador): pasar colección $saldo_puntos.
             Cada item necesita:
               $sp->nombre       → string
               $sp->puntos_saldo → int
               $sp->porcentaje   → int 0-100 (relativo al mayor saldo del listado)
        --}}
        <div class="main-card">
            <div class="card-header-row">
                <span class="card-header-title">Saldo de puntos</span>
                {{-- TODO: ruta gerente.puntos --}}
                <a href="#" class="card-header-action">Ver →</a>
            </div>
            <div style="padding:16px 22px;">
                ...
                <div class="bar-row">
                    ...
                    <div class="bar-track">
                        <div class="bar-fill amber" style="width:{{ $sp->porcentaje }}%"></div>
                    </div>
                    <span class="bar-val">{{ number_format($sp->puntos_saldo, 0, '.', ',') }}</span>
                </div>
                @empty
                <p class="empty-state" style="padding:16px 0;">Sin puntos registrados.</p>
                @endforelse
            </div>
        </div>

        {{-- ── Presolicitudes pendientes ────────────────────────────────
             TODO (controlador): pasar colección $presolicitudes.
             Cada modelo necesita:
               $ps->persona->nombre / apellido
               $ps->estado      → 'pendiente' | 'revision_verificador' | 'revision_coordinador'
               $ps->created_at  → Carbon
               $ps->id          → para el link de detalle
        --}}
        <div class="main-card">
            <div class="card-header-row">
                <span class="card-header-title">Presolicitudes</span>
                {{-- TODO: ruta gerente.presolicitudes --}}
                <a href="#" class="card-header-action">Revisar →</a>
            </div>
            @forelse($presolicitudes ?? [] as $ps)
            <div class="presol-row">
                <div>
                    <div class="presol-name">
                        {{ $ps->persona->nombre }} {{ $ps->persona->apellido }}
                    </div>
                    <div class="presol-time">
                        Hace {{ $ps->created_at->diffForHumans(null, true) }}
                    </div>
                </div>
                <div style="display:flex;align-items:center;gap:10px;">
                    <span class="badge
                        {{ $ps->estado === 'revision_verificador'  ? 'badge-blue'  : '' }}
                        {{ $ps->estado === 'revision_coordinador'  ? 'badge-amber' : '' }}
                        {{ $ps->estado === 'pendiente'             ? 'badge-gray'  : '' }}">
                        @if($ps->estado === 'revision_verificador')     Verificador
                        @elseif($ps->estado === 'revision_coordinador') Coordinador
                        @else                                           Pendiente
                        @endif
                    </span>
                    {{-- TODO: ruta gerente.presolicitudes.show con $ps->id --}}
                    <a href="#" class="card-header-action">Revisar</a>
                </div>
            </div>
            @empty
            <div class="empty-state" style="padding:20px 18px;">
                Sin presolicitudes pendientes.
            </div>
            @endforelse
        </div>
