...
<style>
    ...
    .topbar { display: flex; align-items: flex-start; justify-content: space-between; padding: 24px 32px 16px 32px; }
    ...
    .btn-pdf { padding: 10px 22px; background: #fff; color: #0B1F3A; border: 1.5px solid #E2E8F0; border-radius: 9px; font-size: .88rem; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif; }
    .main-card { margin: 0 32px 32px 32px; background: #fff; border: 1.5px solid #E2E8F0; border-radius: 14px; overflow: hidden; }
    .banner { background: #0B1F3A; padding: 20px 24px; display: flex; justify-content: space-between; align-items: flex-start; }
    ...
    .info-banner { background: #EFF6FF; border-bottom: 1px solid #DBEAFE; padding: 12px 20px; display: flex; align-items: flex-start; gap: 10px; font-size: .82rem; color: #1D4ED8; line-height: 1.6; }
    .pf-table { width: 100%; border-collapse: collapse; font-size: .84rem; }
    .pf-table th { padding: 10px 18px; text-align: left; color: #94A3B8; font-weight: 700; font-size: .68rem; text-transform: uppercase; letter-spacing: .06em; border-bottom: 1px solid #F1F5F9; }
    ...
    .total-bar { background: #0B1F3A; padding: 18px 24px; display: flex; align-items: center; justify-content: space-between; }
...
@media (max-width: 1024px) {
    ...
    .pf-table th,
    .pf-table td { padding: 10px 12px; font-size: .78rem; }

    .credito-bar-wrap { padding: 12px 14px; }

    .info-banner { font-size: .78rem; padding: 10px 14px; }
    ...
    <div class="main-card">
        ...
        <div class="credito-bar-wrap">
            ...
            <div class="credito-bar">
                <div class="credito-fill" style="width:0%;"></div>
            </div>
        </div>
        ...
        <table class="pf-table">
            ...
            <tbody>
                ...
                <tr class="{{ $detalle->recargos > 0 ? 'row-warn' : '' }}">
                    <td class="mono" style="color:#94A3B8;">{{ $loop->iteration }}</td>
                    <td><span class="badge-blue">{{ $detalle->vale->producto->nombre }}</span></td>
                    <td style="font-weight:600;color:#0B1F3A;">{{ $detalle->vale->detalle_vale->nombre_cliente }}</td>
                    <td class="mono" style="color:{{ $detalle->numero_pago_actual >= $detalle->vale->detalle_vale->quincenas ? '#16A34A' : '#D97706' }};">
                        {{ $detalle->numero_pago_actual }}/{{ $detalle->vale->detalle_vale->quincenas }}
                    </td>
                    <td class="mono" style="color:#16A34A;">+${{ $detalle->vale->detalle_vale->monto_comision_calculada }}</td>
                    <td class="mono">${{ $detalle->monto_pago_quincenal }}</td>
                    <td class="mono" style="color:{{ $detalle->recargos > 0 ? '#DC2626' : '#94A3B8' }};">
                        ${{ $detalle->recargos }}
                    </td>
                    <td class="mono" style="font-weight:700;color:{{ $detalle->recargos > 0 ? '#DC2626' : '#0B1F3A' }};">
                        ${{ $detalle->monto_pago_quincenal + $detalle->recargos + $detalle->vale->detalle_vale->monto_comision_calculada }}
                    </td>
                </tr>
                @endforeach
